...
const RecruitmentJobBasedData = ({job}) => {
  const { id } = useParams();
  const { candidates, loading: hookLoading, fetchCandidatesByJob } = useRecruitment();
  ...
  // Tab Counts
  const pipelineCount = candidates.filter(c => c.status !== "Rejected" && c.status !== "Inventory").length;
  const inventoryCount = candidates.filter(c => c.status === "Inventory").length;
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-semibold text-emerald-600">
               <div className="w-5 h-5 bg-emerald-100 rounded-full flex items-center justify-center text-[10px]">✓</div>
               Showing <span className="text-slate-900">{candidates.length} candidates</span>
            </div>

            {/* --- CUSTOM PILL TABS --- */}
            <div className="flex bg-[#F1F3F7] p-1.5 rounded-[22px] gap-1">
              {[
                { id: 'pipeline', label: 'Pipeline', icon: Target, count: pipelineCount },
                { id: 'list', label: 'List', icon: Users, count: null },
                { id: 'analytics', label: 'Analytics', icon: BarChart3, count: null },
                { id: 'inventory', label: 'Inventory', icon: Archive, count: inventoryCount },
              ].map((tab) => (
                <button
                  key={tab.id}
      ...
      <div className="flex-grow relative overflow-hidden">
          ...
          >
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-5 bg-white px-4 py-2 rounded-full shadow-2xl border border-gray-200">
                <button onClick={() => setScale(s => Math.max(s - 0.1, 0.2))} className="p-1.5 hover:bg-gray-100 rounded-md text-gray-600"><ZoomOut className="w-4 h-4" /></button>
                <div className="text-[10px] font-black text-gray-400 w-10 text-center">{Math.round(scale * 100)}%</div>
                <button onClick={() => setScale(s => Math.min(s + 0.1, 2))} className="p-1.5 hover:bg-gray-100 rounded-md text-gray-600"><ZoomIn className="w-4 h-4" /></button>
                <div className="w-px h-4 bg-gray-200 mx-1" />
                <button onClick={resetView} className="p-1.5 hover:bg-gray-100 rounded-md text-gray-600"><Maximize className="w-4 h-4" /></button>
              ...
              <div className="inline-block p-10">
                {hookLoading ? (
                    <div className="flex flex-col items-center justify-center min-h-[500px] w-[1200px] bg-white/40 rounded-3xl border-2 border-dashed border-gray-300">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">Rendering Pipeline...</p>
                    </div>
                ) : (
          ...
          {activeTab === 'analytics' && (
            ...
            <div className="flex flex-col items-center justify-center h-[400px] bg-white rounded-[32px] border border-gray-100">
               <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Recruitment Analytics</p>
            </div>
          )}
      </div>
        ...
        <details className="group">
            <summary className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 list-none">
                <span className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-red-400" /> Rejected Candidates
                </span>
