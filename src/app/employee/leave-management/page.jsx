...
const LeaveManagementPage = () => {
  ...
  // Destructuring separate arrays to prevent tab data pollution
  const {
    leaveRequests,    // Global requests
    myRequests,       // User specific requests
    requestPagination,
    requestLoading,
    fetchMyRequests,
    fetchAllRequests,
  ...
  // 1. Fetch "My Requests" - only when tab is active or user details load
  useEffect(() => {
  ...
  // 2. Fetch "Global Requests" - when filters or page changes
  useEffect(() => {
    const status = activeStatusFilter === "all" ? "" : activeStatusFilter;
    fetchAllRequests({ page: allPage, limit: 10, status });
  }, [allPage, activeStatusFilter, fetchAllRequests]);

  // 3. Filter Logic - points to distinct data sources
      ...
      <Tabs defaultValue="global-pipeline" onValueChange={setCurrentTab} className="w-full">
        <TabsList className="bg-white border border-slate-200 p-1 rounded-xl mb-4">
            <TabsTrigger value="global-pipeline" className="rounded-lg px-6">
              <Globe className="w-4 h-4 mr-2" /> Global Pipeline
            </TabsTrigger>
        ...
        <TabsContent value="global-pipeline" className="space-y-6 outline-none">
          <div className="bg-white border border-slate-200 rounded-xl p-4 flex gap-4 items-center justify-between ">
